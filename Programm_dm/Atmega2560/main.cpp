...
#define encoder_drive 280
#define encoder_drive_half 140
#define encoder_drive_after_ir_back 40
#define encoder_turn_right 210
...
#define velocity_drive 180
#define velocity_drive_ir_slow 180
#define velocity_turn 180
#define velocity_power_turn 210
...
#define color_victm 6
#define color_victm_high 10
#define color_victm_low 2
#define color_checkpoint_threshold 100

#define greyscale_black_threshold 100

/*----------servo values----------*/
#define servo_home_pulse 1000
...
unsigned char are_exit[area_amount][area_amount];
...
int main(void) {
	...
	while(1) {
		 ...
		 display_data();
		 if (checkpoint && position_x >= (start_position - exit_radius) && position_x <= (start_position + exit_radius) && position_y >= (start_position - exit_radius) && position_y <= (start_position + exit_radius) && !((direction + turn_direction) % 4) && !area && /*(map[start_position][start_position][0] & wall_map) == wall &&*/ victm_counter && !lack_of_progress_flag) exit();
...
void display_data(void) {
	data[0] = (direction % 4);//ir_front_right;
	data[1] = position_x;//ir_front_left;
	data[2] = position_y;//ir_right_front;
	data[3] = turn_direction;//ir_right_back;
	data[4] = wall_rel;//ir_back_right;
	data[5] = wall;//ir_back_left;
	data[6] = 0;//ir_left_front;
	data[7] = 0;//ir_left_back;
	data[8] = 0;
	data[9] = 0;
}
...
void drive() {
	...
	while(encoder_counter < encoder_drive && !lack_of_progress_flag) {
		...
		forward(velocity_drive, velocity_drive);
		...
		if (ir_front_right < ir_wall_optimal || ir_front_left > ir_wall_optimal || ultrasonic_front < ultrasonic_wall_theshold_front) encoder_counter_ir_contact_right = 0;
		...
		if (touch_front_right && !touch_front_left && !lack_of_progress_flag) {
			stop();
			encoder_counter_temp_contact = encoder_counter;
			encoder_counter = 0;
		...
		if ((ir_front_left < ir_wall_optimal || ir_front_right > ir_wall_optimal || ultrasonic_front < ultrasonic_wall_theshold_front) && !lack_of_progress_flag) encoder_counter_ir_contact_right = 0;
		...
		if (touch_front_left && !touch_front_right && !lack_of_progress_flag) {
			stop();
			encoder_counter_temp_contact = encoder_counter;
			encoder_counter = 0;
			while (encoder_counter < encoder_drive_after_contact && !lack_of_progress_flag) backward(velocity_align_drive, velocity_align_drive);
	...
	if (ramp && !lack_of_progress_flag) {
		...
		if (recover_start_tile_counter) {
			position_x_est += dx[direction % 4];
			position_y_est += dy[direction % 4];
		}
...
void handle_map(void) {
	...
	} else if (!lack_of_progress_flag) map[position_x][position_y][area] |= (wall | known_map);
	...
	if (mode & 0b00000001 && !lack_of_progress_flag) {
		...
		if (wall_quick_rel & wr && !lack_of_progress_flag) {
			...
			if (ir_left_front > ir_left_back && !lack_of_progress_flag) {
				while (ir_left_front > ir_left_back && ir_left_front > ir_wall_threshold && ir_left_back > ir_wall_threshold && encoder_counter < encoder_align_max_turn && !lack_of_progress_flag) turn_left(velocity_align_turn);
			} else if (ir_left_back > ir_left_front && !lack_of_progress_flag) {
				while (ir_left_back > ir_left_front && ir_left_front > ir_wall_threshold && ir_left_back > ir_wall_threshold && encoder_counter < encoder_align_max_turn && !lack_of_progress_flag) turn_right(velocity_align_turn);
			}
			...
			if (ir_front_right > ir_front_left && !lack_of_progress_flag) {
				while (ir_front_right > ir_front_left && ir_front_right > ir_wall_threshold && ir_front_left > ir_wall_threshold && encoder_counter < encoder_align_max_turn && !lack_of_progress_flag) turn_left(velocity_align_turn);
			} else if (ir_front_left > ir_front_right && !lack_of_progress_flag) {
				while (ir_front_left > ir_front_right && ir_front_right > ir_wall_threshold && ir_front_left > ir_wall_threshold && encoder_counter < encoder_align_max_turn && !lack_of_progress_flag) turn_right(velocity_align_turn);
			}
		}*/
	}
	stop();
	if (mode & 0b00000010 && !lack_of_progress_flag) {
		encoder_counter = 0;
...
void detect_wall() {
	...
	if (recover_start_tile_counter) wall_est = calculate_absolute_wall(direction, wall_rel);
	led_array_off;
	led_array |= wall_rel;
	display_data();
}
